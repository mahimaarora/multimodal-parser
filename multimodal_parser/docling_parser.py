"""
Docling Parser for Multimodal Document Parsing

Simple parser to extract text, tables, and images from PDF documents using Docling.
"""
...
class DoclingParser:
    """Simple PDF parser using Docling for text, table, and image extraction."""
    ...
    def __init__(
        ...
        """
        Initialize the parser with PDF pipeline options.
        ...
        Args:
            ...
            google_api_key: Google API key for descriptions (falls back to GOOGLE_API_KEY env var)
        ...
        if self.api_key and generate_descriptions:
            ...
            logger.info("ModelManager initialized for AI descriptions")
        elif generate_descriptions and not self.api_key:
            logger.warning("GOOGLE_API_KEY not set. AI descriptions disabled.")
    ...
    def _create_image_chunk(self, item: PictureItem, document, idx: int, source_doc: str,
        ...
        if image_base64:
            ...
            try:
                ...
                image_path = str(image_path.resolve())  # Use absolute path
        ...
        # Infer image type
        image_type = None
        if self.model_manager and (annotation or caption):
            image_type = self.model_manager.infer_image_type(annotation, caption)
    ...
    def _describe_table(self, df, caption: str = "") -> str:
        ...
        return self.model_manager.describe_table(
            headers=headers,
            num_rows=len(df),
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m multimodal_parser.docling_parser <pdf_path>")
        sys.exit(1)
    ...
    for chunk in chunks:
        print(f"[{chunk.chunk_type.upper()}] {chunk.chunk_id} - Page {chunk.source_page}")
        if chunk.content:
            print(f"  Content: {chunk.content[:100]}...")
        if hasattr(chunk, 'num_rows'):
            print(f"  Table: {chunk.num_rows}x{chunk.num_cols}")
        if hasattr(chunk, 'image_format'):
            print(f"  Format: {chunk.image_format}, Type: {chunk.image_type}")
        print()
